...
namespace Greenheck_Project.Database
{
    class DataRetrievalClass
    {
        ...
        private const string dbA = @"Data Source = (LocalDB)\MSSQLLocalDB;AttachDbFilename=H:\Private\CIS 480 Capstone\CIS 480 Capstone\Greenheck Project\Greenheck Project\Database\Database1.mdf";
        ...
        {
            List<Quarter> thing = new List<Quarter>();
            SqlCommand fetch = new SqlCommand();
            fetch.Connection = GetConn();

            fetch.CommandText = "SELECT FiscalYear, Quarter, COUNT(Cancelled) 'Cancelled', COUNT(Completed) 'Completed', COUNT(Delayed) 'Delayed', COUNT(Ongoing) 'InProgress', COUNT(NotStarted) 'NotStarted' FROM FocusCommentsTable LEFT JOIN StatusTable ON FocusCommentsTable.StatusID = StatusTable.StatusID";
            ...
            while (list.Read())
            {
                ...
                thus.statusAxed = Convert.ToInt32(list["Cancelled"]);
                thus.statusComplete = Convert.ToInt32(list["Completed"]);
                thus.statusDelay = Convert.ToInt32(list["Delayed"]);
                thus.statusInProg = Convert.ToInt32(list["InProgress"]);
                thus.statusNotStart = Convert.ToInt32(list["NotStarted"]);
